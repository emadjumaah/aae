...
sugEl.innerHTML = SUGGESTIONS.map(
  (s) =>
...
sugEl.querySelectorAll<HTMLButtonElement>(".sug-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    (document.getElementById("liveInput") as HTMLInputElement).value =
      btn.dataset.s!;
    runLive();
  });
});
...
(document.getElementById("liveInput") as HTMLInputElement).addEventListener(
  "keydown",
  (e) => {
...
function runLive() {
  const input = (
    document.getElementById("liveInput") as HTMLInputElement
  ).value.trim();
  if (!input) return;
...
exGrid.innerHTML = EXAMPLES_DATA.map(
  (ex, i) => `
...
bIntentEl.innerHTML = INTENTS.map(
  (i) => `<option value="${i}">${i}</option>`,
).join("");
bRootEl.innerHTML = ROOTS.map(
  (r) =>
    `<option value="${esc(r.arabic)}">${esc(r.arabic)} (${esc(r.latin)}) — ${esc(r.semanticField)}</option>`,
).join("");
bPatternEl.innerHTML = PATTERNS.map(
  (p) =>
    `<option value="${p.operator}">${p.operator} — ${esc(p.wazn)}</option>`,
).join("");
...
document.getElementById("bRun")!.addEventListener("click", () => {
  ...
  const modsRaw = (
    document.getElementById("bMods") as HTMLInputElement
  ).value.trim();
  ...
  const token: AlgebraToken = {
    intent,
    root: rootEntry.arabic,
    rootLatin: rootEntry.latin,
    pattern,
    modifiers,
  };
...
// ─── Dictionary ─────────────────────────────────────────────────────────────
document.getElementById("rootsGrid")!.innerHTML = ROOTS.map(
  (r) => `
  <div class="dict-card">
...
document.getElementById("patternsGrid")!.innerHTML = PATTERNS.map(
  (p) => `
  <div class="dict-card">
