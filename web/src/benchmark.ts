...
// ─── Hero stats (id="hero-stats") ───────────────────────────────────────────
{
  ...
  const stats = [
    { value: `${(overall.intentAcc * 100).toFixed(1)}%`, label: 'Intent Accuracy', sub: `vs GPT-4o ${(llmBaseline.models['GPT-4o'].intentAcc * 100).toFixed(0)}%` },
    { value: `${(overall.actionAcc * 100).toFixed(1)}%`, label: 'Action Accuracy', sub: `vs GPT-4o ${(llmBaseline.models['GPT-4o'].actionAcc * 100).toFixed(0)}%` },
    { value: `${(overall.fullAcc * 100).toFixed(1)}%`, label: 'Full Match', sub: `${R.totalCases} test cases` },
    { value: `~${Math.round(overall.avgDurationUs)}µs`, label: 'Avg Latency', sub: `${(llmBaseline.models['GPT-4o'].avgLatencyMs / (overall.avgDurationUs / 1000)).toFixed(0)}× faster than GPT-4o` },
...
// ─── Meta ───────────────────────────────────────────────────────────────────
document.getElementById('meta')!.textContent = `Engine v${R.engineVersion} · ${R.totalCases} single cases · ${consistency.length} consistency groups · ${bilingual.length} bilingual pairs · ${new Date(R.timestamp).toLocaleDateString()}`;
...
// ─── Accuracy charts (id="chart-intent", id="chart-action") ────────────────
{
  ...
  // Intent chart
  new Chart((document.getElementById('chart-intent') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: {
      labels: allLabels,
      datasets: [{
        label: 'Intent Accuracy',
        data: [overall.intentAcc, ...models.map(m => llmBaseline.models[m].intentAcc)],
        backgroundColor: allLabels.map((_, i) => i === 0 ? GOLD : CYAN),
      }],
    },
    options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { min: 0.8, max: 1 } } },
  });

  // Action chart
  new Chart((document.getElementById('chart-action') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: {
      labels: allLabels,
      datasets: [{
        label: 'Action Accuracy',
        data: [overall.actionAcc, ...models.map(m => llmBaseline.models[m].actionAcc)],
        backgroundColor: allLabels.map((_, i) => i === 0 ? GOLD : GREEN),
      }],
    },
    options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { min: 0.7, max: 1 } } },
  });
}
...
// ─── Comparison table (id="comp-body") ──────────────────────────────────────
{
  ...
  function win(ours: number, theirs: number) { return ours >= theirs ? 'win' : 'lose'; }
...
{
  const labels = ['Intent', 'Action', 'Consistency', 'Bilingual', 'Explainability', 'Cost Eff.'];
  ...
  Object.entries<any>(llmBaseline.models).forEach(([name, m], i) => {
    datasets.push({
      label: name, data: [m.intentAcc, m.actionAcc, m.consistencyRate, m.bilingualParity, 0, m.costPer1000 < 1 ? 0.8 : 0.2],
      borderColor: modelColors[i], backgroundColor: 'transparent', pointBackgroundColor: modelColors[i], borderDash: [4, 2],
    });
  });

  new Chart((document.getElementById('chart-radar') as HTMLCanvasElement).getContext('2d')!, {
...
{
  new Chart((document.getElementById('chart-ops') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: {
      labels: ['Deterministic', 'Explainable', 'Offline', 'Privacy'],
      datasets: [
        { label: 'Arabic Algebra', data: [1, 1, 1, 1], backgroundColor: GOLD },
    ...
    options: { responsive: true, scales: { y: { min: 0, max: 1, ticks: { callback: (v: number) => v === 1 ? 'Yes' : v === 0 ? 'No' : '' } } }, plugins: { legend: { position: 'top' } } },
  });
}
...
// ─── Latency chart (id="chart-latency") ─────────────────────────────────────
{
  ...
  new Chart((document.getElementById('chart-latency') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: { labels: allLabels, datasets: [{ label: 'Latency (ms)', data: latencies, backgroundColor: allLabels.map((_, i) => i === 0 ? GOLD : CYAN) }] },
    options: { responsive: true, indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { type: 'logarithmic', title: { display: true, text: 'ms (log scale)' } } } },
  });
}
...
// ─── Cost chart (id="chart-cost") ───────────────────────────────────────────
{
  ...
  new Chart((document.getElementById('chart-cost') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: { labels: allLabels, datasets: [{ label: 'Cost per 1K ($)', data: costs, backgroundColor: allLabels.map((_, i) => i === 0 ? GOLD : ROSE) }] },
    options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { title: { display: true, text: 'USD per 1,000 requests' } } } },
  });
}
...
// ─── Per-category (id="chart-categories") ───────────────────────────────────
{
  ...
  new Chart((document.getElementById('chart-categories') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: {
      labels: catLabels,
      datasets: [
    ...
    options: { responsive: true, plugins: { legend: { position: 'top' } }, scales: { y: { min: 0.75, max: 1 } } },
  });
}
...
// ─── Consistency chart (id="chart-consistency") ─────────────────────────────
{
  ...
  new Chart((document.getElementById('chart-consistency') as HTMLCanvasElement).getContext('2d')!, {
    type: 'doughnut',
    data: {
      labels: ['Fully Consistent', 'Diverged'],
      datasets: [{ data: [consistent, diverged], backgroundColor: [GREEN, RED] }],
    },
...
// ─── Consistency detail (id="consistency-detail") ───────────────────────────
{
  ...
  el.innerHTML = '<h4 style="color:var(--cyan);margin-bottom:12px;">Paraphrase Groups</h4>' +
    consistency.map(g => {
...
// ─── Bilingual charts (id="chart-bilingual", id="chart-bilingual2") ────────
{
  ...
  new Chart((document.getElementById('chart-bilingual') as HTMLCanvasElement).getContext('2d')!, {
    type: 'doughnut',
    data: { labels: ['Parity', 'Diverged'], datasets: [{ data: [parityCount, divergedCount], backgroundColor: [GREEN, RED] }] },
    options: { responsive: true, plugins: { legend: { position: 'bottom' } } },
  });
  ...
  new Chart((document.getElementById('chart-bilingual2') as HTMLCanvasElement).getContext('2d')!, {
    type: 'bar',
    data: {
      labels: ['English', 'Arabic'],
      datasets: [
        { label: 'Correct', data: [enCorrect, arCorrect], backgroundColor: GREEN },
        { label: 'Incorrect', data: [bilingual.length - enCorrect, bilingual.length - arCorrect], backgroundColor: RED },
      ],
    },
    options: { responsive: true, plugins: { legend: { position: 'top' } }, scales: { x: {}, y: { stacked: true } } },
  });
}
...
// ─── Bilingual detail (id="bilingual-detail") ──────────────────────────────
{
  ...
  el.innerHTML = '<h4 style="color:var(--cyan);margin-bottom:12px;">Pair Details</h4>' +
    bilingual.map(b => {
      const ok = b.parity;
...
// ─── Case explorer (id="case-filters", id="case-list") ─────────────────────
{
  ...
  const filterCategories = ['all', 'pass', 'fail', 'intent', 'action', 'disambiguation', 'adversarial'];

  filtersEl.innerHTML = filterCategories.map(f =>
  ...
  function renderCases(filter: string) {
    ...
    else if (filter !== 'all') filtered = cases.filter(c => c.category === filter);
